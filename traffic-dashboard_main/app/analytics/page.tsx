...
import { TrendingUp, Leaf, Droplets, HeartPulse, Globe, Banknote, Zap, ArrowUpRight, BarChart3, Server } from "lucide-react"
...
export default function AnalyticsPage() {
  ...
  const { junctionDowntimeMs, activeFaultStartTimes, sessionStartTime } = useHardwareHealth()
  ...
  const [waitTimes, setWaitTimes] = useState<number[]>(() => {
    if (typeof window !== "undefined") {
      const s = localStorage.getItem("intelliflow_wait_times")
      return s ? JSON.parse(s) : [15]
    }
    return [15]
  })
  ...
  const [cumulativeEvpTime, setCumulativeEvpTime] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const s = localStorage.getItem("intelliflow_evp_time")
  ...
  useEffect(() => {
    ...
    Object.entries(jData).forEach(([jid, j]: [string, any]) => {
      ...
      if (prevPhase && prevPhase !== currentPhase && prevTime > 0) {
      ...
      const emergencyStart = activeEmergencies.current[jid]
      const currentSimTime = simulationData.simulation_time || 0

      if (isEmergency && !emergencyStart) {
        activeEmergencies.current[jid] = currentSimTime || 0.1
      } else if (!isEmergency && emergencyStart) {
        const duration = currentSimTime - emergencyStart
        newlyFreedEvpTime += Math.max(0, 60 - duration)
  ...
  // --- Derived Metrics ---
  const numJunctions = simulationData?.junctions ? Object.values(simulationData.junctions).length : 0
  ...
  const lives_saved = 0.63 * evp_time_minutes * 0.07
  ...
  const [systemUptimePct, setSystemUptimePct] = useState<number>(100)
  const [avgDowntimeSec, setAvgDowntimeSec] = useState<number>(0)

  // Store latest values in refs so the interval doesn't need them as deps
  const simDataRef = useRef(simulationData)
  const downtimeRef = useRef(junctionDowntimeMs)
  const activeFaultsRef = useRef(activeFaultStartTimes)
  const sessionStartRef = useRef(sessionStartTime)
  useEffect(() => { simDataRef.current = simulationData }, [simulationData])
  useEffect(() => { downtimeRef.current = junctionDowntimeMs }, [junctionDowntimeMs])
  useEffect(() => { activeFaultsRef.current = activeFaultStartTimes }, [activeFaultStartTimes])
  useEffect(() => { sessionStartRef.current = sessionStartTime }, [sessionStartTime])

  // Interval set up once — never restarts, never triggers animation spuriously
  useEffect(() => {
    const compute = () => {
      const activeJids = simDataRef.current?.junctions ? Object.keys(simDataRef.current.junctions) : []
      const elapsed = Math.max(1, Date.now() - sessionStartRef.current) / 1000
      
      const ongoingMs = activeJids.reduce((sum, jid) => {
        const start = activeFaultsRef.current[jid]
        return sum + (start ? (Date.now() - start) : 0)
      }, 0)

      const committed = (activeJids.reduce((sum, jid) => sum + (downtimeRef.current[jid] || 0), 0) + ongoingMs) / 1000
      const avgDown = activeJids.length > 0 ? committed / activeJids.length : 0
      const pct = parseFloat(Math.max(0, Math.min(100, 100 - (avgDown / elapsed) * 100)).toFixed(2))
      const downSec = parseFloat(avgDown.toFixed(1))
      // Only update state (and trigger NumberTicker animation) if value actually changed
      setSystemUptimePct(prev => prev !== pct ? pct : prev)
      setAvgDowntimeSec(prev => prev !== downSec ? downSec : prev)
    }
    compute() // run once on mount
    const interval = setInterval(compute, 10_000) // re-sample every 10s only
    return () => clearInterval(interval)
  }, []) // empty deps — interval is created once, reads data via refs
  ...
  const metricCards = [
    ...
    },
    {
      ...
      subtext: `Increase in traffic flow vs fixed timer`
    },
    {
    ...
    {
      title: "System Uptime",
      numeric: systemUptimePct,
      suffix: "%",
      decimals: 2,
      icon: <Server className="w-6 h-6 text-violet-500" />,
      subtext: `Avg downtime per agent: ${Math.floor(avgDowntimeSec / 60)}m ${Math.floor(avgDowntimeSec % 60)}s`
    },
  ]
      ...
      <motion.div variants={itemVariants} className="w-full bg-slate-950 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-950 via-blue-900 to-cyan-800 rounded-[2.5rem] p-8 sm:p-12 text-white shadow-[0_12px_40px_rgba(0,0,0,0.25)] overflow-hidden relative border border-blue-900/50">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-6 leading-tight tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white via-blue-100 to-indigo-300 drop-shadow-sm">
            Scaling AI over 1000 junctions saves almost <span className="text-cyan-400 drop-shadow-[0_0_12px_rgba(34,211,238,0.4)]">₹110 crore</span> annually.
          </h2>
