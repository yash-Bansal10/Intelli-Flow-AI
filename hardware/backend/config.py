...
# ── Network ──────────────────────────────────────────────────────────────────
ESP32_CAM_URL   = "http://192.168.43.235/"   # ESP32-CAM MJPEG stream URL (Native camera daemon root)
ESP32_CTRL_URL  = "http://192.168.43.126"    # ESP32 Controller base IP 
...
DQN_WEIGHTS_PATH = "../finetuning/models/model_main.weights.h5"
...
# ── Backend Timing ────────────────────────────────────────────────────────────
TICK_INTERVAL_S    = 0.05   # Run tick as fast as possible (unshackle from 1 FPS)
ESP32_POST_TIMEOUT = 2.0    # Max seconds to wait for ESP32 to respond (MicroPython is slow!)
CAM_READ_TIMEOUT   = 5.0    # Seconds to wait for a frame before declaring disconnected
MIN_GREEN_TIME     = 3.0    # Reduced from 20s to 3s for fast-paced Hackathon desk presentation
EMERGENCY_HOLD     = 10.0   # Seconds to hold green after ambulance leaves camera
...
PHASE_TO_ESP32 = {
    ...
    "EW-Green":    "EW_GREEN",
    "All-Red":     "ALL_RED",
    "Fixed-Time":  "ALL_RED", # Fallback for edge hardware missing an internal clock
...
YOLO_CONF_THRESHOLD = 0.20   # Lowered to heavily boost detection rate on moving toy cars
